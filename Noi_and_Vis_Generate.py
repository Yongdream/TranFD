...
def Get_Data(path, T_begin, T_end):  # 获取指定路径的原始数据、相关系数及压差绝对值矩阵，并输出相关系数最小值及压差绝对值最大值
    ...
    ### 修改过程：1、第72行噪声添加函数(仅噪声) 2、第103行重新赋值函数(仅粘滞) 3、第161行目标目录名 4、第164行噪声sigma大小(仅噪声) 5、第184行函数选择
    U_5001 = Add_Vis(U_5001, 3.8)  # 粘滞（记得修改存储路径）
    ...
    U_5001 = Add_Vis(U_5001, Vis_value)  # 粘滞
    data = []
    for i in range(0, len(U_5001)):  # 原数据矩阵
        data.append([U_5001[i], U_5002[i], U_5003[i], U_5004[i], U_5005[i], U_5006[i], U_5007[i], U_5008[i], U_5009[i],
                      U_5010[i], U_5011[i], U_5012[i], U_5013[i], U_5014[i], U_5015[i], U_5016[i], U_5017[i], U_5018[i],
    ...
    for i in range(0, len(data_corr)):
        for j in range(0, len(data_corr[0])):
            if data_corr[i][j] < 0:
                data_corr[i][j] = 0
    ...
    #     All_list.append([U_5001[i], U_5002[i]])
    #
    # df1 = pd.DataFrame(data=All_list,
    #                    columns=['Fault cell', 'Normal cell'])
    # df1.to_csv('./Result_csv/Low_Acc.csv', index=False)

    return data_feature, U_max, U_min, corr_min, diff_max

def Data_Origin_Saved_Npy(data_feature, path, Condition, Fault, T_begin, T_end):  # 数据矩阵/原始路径/工况词典/故障词典
    ...
    file_name = d_c + "_" + d_f + "_" + "s0_05" + "_" + path[-8:-4]  # 原文件日期部分
    if d_f == "Vis":
        file_name = d_c + "_" + d_f + "_" + str(T_begin) + "-" + str(T_end) + "_" + path[-8:-4]

    dir_path = "./Npy/" + d_c + "/" + d_f
...
if __name__ == '__main__':
    path_n = "./Origin/US06/Normal/US06_Nor_0930.csv"  # 文件名命名最后4位需为日期，且同一工况同一故障最好不得在一天内重复

    len_d = Get_Data_Len(path_n)

    ### 粘滞
    for T_begin in range(0, len_d - 900, 600):
        data_feature, U_max, U_min, corr_min, diff_max = Get_Data(path_n, T_begin, T_begin + 900)
        print("Data Get OK!")
        print("U_max:" + str(U_max) + " U_min:" + str(U_min) + " corr_min:" + str(corr_min) + " diff_max:" + str(
            diff_max))
        Condition = ["FUDS", "UDDS", "US06"]  # 工况词典
        Fault = ["Cor", "Isc", "Noi", "Nor", "Vis"]  # 相关性/微短路/噪声/正常/粘滞
        # Data_Origin_Saved_Npy(data_feature, path_n, Condition, Fault, T_begin + 300, T_begin + 900)
    ...
    # Condition = ["FUDS", "UDDS", "US06"]  # 工况词典
    # Fault = ["Cor", "Isc", "Noi", "Nor", "Vis"]  # 相关性/微短路/噪声/正常/粘滞
    # # Data_Origin_Saved_Npy(data_feature, path_n, Condition, Fault, 0, len_d)
