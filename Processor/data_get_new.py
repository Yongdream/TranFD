...
def calc_corr(a, b, a_avg, b_avg):  # 计算相关系数
    # a_avg = sum(a) / len(a)
    # b_avg = sum(b) / len(b)
...
def calc_corr_group(a, AVE, win):  # 以时间窗截取的方式进行循环计算
    a_avg = sum(a) / len(a)
    b_avg = sum(AVE) / len(AVE)
    b = np.zeros_like(a[win:])  # 构造目标形状的0矩阵
    for i in range(win, len(a[:])):  # 时间戳从win开始进行截取
        b[i - win] = calc_corr(a[i - win:i], AVE[i - win:i], a_avg, b_avg)  # 对该时间戳及其之前长度为win的数据进行截取
    return b
    ...
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file.endswith(".csv"):  # 只处理 CSV 文件
                ...
                print("Processing file:", path_n)

                data_feature, U_max, U_min, corr_min, diff_max = Get_Data(path_n)
                print(
                    "U_max:" + str(U_max) + " U_min:" + str(U_min) + " corr_min:" + str(corr_min) + " diff_max:" + str(
                        diff_max))
                print("Data Get OK!")
